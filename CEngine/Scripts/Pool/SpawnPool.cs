...
namespace CYM.Pool
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        private bool _dontDestroyOnLoad = false;  // Property backer and used by GUI.
        ...
        /// </summary>
        public float MaxParticleDespawnTime { get; set; } = 300;
        ...
        /// </summary>
        public InstantiateDelegate InstantiateDelegates { get; set; }
        ...
        /// Unity's Instantiate.
        /// </summary>
        public DestroyDelegate DestroyDelegates { get; set; }
        ...
        /// </summary>
        private Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot)
        {
        ...
        private Transform Spawn(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent)
        {
            return Spawn(prefab.transform, pos, rot, parent);
        }

        private Transform Spawn(GameObject prefab, Vector3 pos, Quaternion rot)
        {
        ...
        private void Despawn(Transform instance)
        {
            if (GO == null)
                return;
            if (!GO.activeSelf)
            ...
            instance.position = SysConst.VEC_FarawayPos;
        ...
        /// just before despawn.
        /// </summary>
        private void Despawn(Transform instance, Transform parent)
        {
            // Spawn the new instance (Note: prefab already set in PrefabPool)
        ...
        private void Despawn(Transform instance, float seconds)
        {
            if (GO == null) return;
        ...
        /// just before despawn.
        /// </summary>
        private void Despawn(Transform instance, float seconds, Transform parent)
        {
        ...
        #region my extend
        HashSet<GameObject> PrespawnKey = new HashSet<GameObject>();
        public IEnumerator<float> PrespawnAsyn(GameObject go, int count = 1, Vector3? position = null, Quaternion? quaternion = null, Transform paraent = null)
        {
            if (PrespawnKey.Contains(go))
                yield break;
            PrespawnKey.Add(go);
            List<GameObject> objs = new List<GameObject>();
            for (int i = 0; i < count; ++i)
            {
                var temp = Spawn(go, position, quaternion, paraent);
                objs.Add(temp);
        ...
        public void Prespawn(GameObject go, int count = 1, Vector3? position = null, Quaternion? quaternion = null, Transform paraent = null)
        {
            if (PrespawnKey.Contains(go))
                return;
            PrespawnKey.Add(go);
        ...
        public Transform SpawnTrans(GameObject go, Vector3? position = null, Quaternion? quaternion = null, Transform paraent = null)
        {
            if (go == null)
                return null;
            Transform ret = null;
            if (paraent == null)
                ret = Spawn(go, position.HasValue ? position.Value : SysConst.VEC_FarawayPos, quaternion.HasValue ? quaternion.Value : Quaternion.identity);
            else
                ret = Spawn(go, position.HasValue ? position.Value : SysConst.VEC_FarawayPos, quaternion.HasValue ? quaternion.Value : Quaternion.identity, paraent);
            return ret;
        }
        public GameObject Spawn(GameObject go, Vector3? position = null, Quaternion? quaternion = null, Transform paraent = null)
        {
            if (go == null) return null;
            Transform trans = SpawnTrans(go, position, quaternion, paraent);
            if (trans != null) return trans.gameObject;
            return null;
        }
        public void Despawn(BaseMono mono, float delayTime = 0.0f)
        {
            if (delayTime > 0.0f)
                Despawn(mono.Trans, delayTime);
            else
                Despawn(mono.Trans);
        }
        public void Despawn(GameObject mono, float delayTime = 0.0f)
        {
            if (mono == null)
                return;
            if (delayTime > 0.0f)
                Despawn(mono.transform, delayTime);
            else
                Despawn(mono.transform);
        }
        /// <summary>
        /// 将对象的父节点变成对象池旗下
        /// </summary>
        /// <param name="mono"></param>
        public void GiveBack(BaseMono mono)
        {
            mono.Trans.parent = transform;
        }
