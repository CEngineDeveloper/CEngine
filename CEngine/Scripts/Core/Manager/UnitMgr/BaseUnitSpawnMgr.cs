...
namespace CYM
{
        ...
        where TConfig :  TDBaseData, new()
    {
        ...
        //是否作为玩家管理器
        public virtual bool IsPlayerSpawnMgr => false;
        //不读取配置，适用于动态创建的军团，读取配置则适用于配置的城市，国家等
        protected virtual bool IsNoConfig => false;
        protected virtual string UnitPrefab => null;
        ...
        public virtual TUnit SpawnNew(string tdidOrPrefab, [DefaultValue(nameof(UnitSpawnParam.Default))] UnitSpawnParam param)
        {
        ...
        //通常的Spawn
        public virtual TUnit Spawn(string tdidOrPrefab, [DefaultValue(nameof(UnitSpawnParam.Default))]UnitSpawnParam param)
        {
            ...
            else
            {
                ...
                if (!param.prefab.IsInv()) goPrefab = BundleCacher.Get(param.prefab,false);
                else goPrefab = BundleCacher.Get(param.tdid, false);
                if (goPrefab == null) goPrefab = BundleCacher.Get(UnitPrefab, false);
                if (goPrefab == null)
                {
                    CLog.Error("单位没有Prefab，Config TDID:{0}", config.TDID);
                    return null;
                }
            }
        ...
        public virtual void SpawnAdd(TUnit chara, [DefaultValue(nameof(UnitSpawnParam.Default))] UnitSpawnParam param)
        {
        ...
        #region virtual
        protected virtual string GoldTDID => "Gold";
