...
namespace CYM
{
    public class Builder
    {
        ...
        // 根据文件夹获得名称
        static Tuple<string, string> GetABNameWhole(DLCItem item, BuildRuleConfig data)
        {
            string source = data.Name.ToLower();
            return new Tuple<string, string>((item.Name + "_" + source).ToLower(), source.ToLower());
        }
        static Tuple<string, string> GetABNameFold(DLCItem item, BuildRuleConfig data,string foldName)
        {
            string source = Path.GetDirectoryName(foldName).ToLower();
            return new Tuple<string, string>((item.Name + "_" + source).ToLower(), source);
        }
        // 获得共享资源名称
        ...
        static void BuildDLCConfigInternal(DLCItem dlcItem)
        {
            ...
            string constPath = dlcItem.ConstPathBuild;
            string manifestPath = dlcItem.ManifestPathBuild;
            string dlcItemPath = dlcItem.ConfigPathBuild;
        ...
        private static List<AssetBundleBuild> GenerateAssetBundleBuildData(DLCItem dlc)
        {
            ...
            //建立其他文件
            List<BuildRuleConfig> tempBuildDatas = dlc.FinalBuildRule;
            ...
            foreach (var item in tempBuildDatas)
            {
                ...
                if (item.BuildRuleType == BuildRuleType.Whole)
                    buildRule = new BuildAssetsWithWhole(dlc, item);
                else if (item.BuildRuleType == BuildRuleType.Path)
                ...
                else if(item.BuildRuleType == BuildRuleType.Fold)
                    buildRule = new BuildAssetsWithFold(dlc, item);
                Rules.Add(buildRule);
            }
        ...
        class BuildAssetsWithWhole : BuildRule
        {
            public BuildAssetsWithWhole(DLCItem dlc, BuildRuleConfig config) : base(dlc, config)
            {
            }
            public override void Build()
            {
                ...
                Tuple<string, string> bundleName = GetABNameWhole(DLCItem, Config);
                for (int i = 0; i < files.Count; i++)
                {
                    var item = files[i];
        ...
        class BuildAssetsWithFold : BuildRule
        {
            public BuildAssetsWithFold(DLCItem dlc, BuildRuleConfig config) : base(dlc, config)
            {
            }
            public override void Build()
            {
                var foldes = Directory.GetDirectories(Config.FullSearchPath);
                foreach(var fold in foldes)
                {
                    List<string> packedAsset = new List<string>();
                    var bundleName = GetABNameFold(DLCItem, Config, fold);
                    var files = GetFilesWithoutDirectories(fold);
                    for (int i = 0; i < files.Count; i++)
                    {
                        var item = files[i];
                        if (EditorUtility.DisplayCancelableProgressBar(string.Format("Packing... [{0}/{1}]", i, files.Count), item, i * 1f / files.Count))
