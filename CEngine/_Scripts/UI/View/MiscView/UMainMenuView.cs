...
namespace CYM.UI
{
    public class UMainMenuView : UUIView 
    {
        ...
        {
            base.OnCreatedView();
            BntStartGame?.Init(new UButtonData { NameKey = "新游戏", OnClick = OnClickNewGame, IsInteractable = IsNewGame });
            BntContinueGame?.Init(new UButtonData { NameKey = "继续游戏", OnClick = OnContinueGame, IsInteractable = IsContinueGame });
            BntTutorial?.Init(new UButtonData { NameKey = "游戏教程",OnClick = OnTutorial });
            BntSettings?.Init(new UButtonData { NameKey = "设置", OnClick = OnClickSettings });
            BntCredits?.Init(new UButtonData { NameKey = "制作人", OnClick = OnClickCredits });
            BntQuit?.Init(new UButtonData { NameKey = "退出", OnClick = OnClickExitGame });
            BntWebsite?.Init(new UButtonData { NameKey = "Websit", IsTrans = false, OnClick = OnClickWebsite });
